...
`;
    ...
    cleanups.push(() => {
      footerAudio?.removeEventListener('timeupdate', updateSubtitles)
      footerAudio?.removeEventListener('seeked', updateSubtitles)
      footerAudio?.removeEventListener('play', updateSubtitles)
      footerAudio?.removeEventListener('ended', showFinalCue)
    })
    ...
    if (subtitlesUrl) {
      ...
      cleanups.push(() => {
        footerAudio?.pause()
        try { if (footerAudio) footerAudio.currentTime = 0 } catch {}
        footerAudio = null
        stopVoiceVideo()
      })
    }
...
`;
    ...
    cleanups.push(() => {
      ...
      footerAudio?.removeEventListener('seeked', updateSubtitles)
      footerAudio?.removeEventListener('play', updateSubtitles)
      footerAudio?.removeEventListener('ended', transitionToFinish)
    })
...
`
  ...
  if (cleanups.length) {
    return { element: card, cleanup: () => cleanups.forEach((fn) => fn()) }
  }
